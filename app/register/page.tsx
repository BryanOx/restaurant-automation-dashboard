...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await register(tenantName, email);
    } catch (err: unknown) {
      console.log('[Register] Caught error:', err);
      ...
      if (err && typeof err === 'object' && 'response' in err) {
        ...
        console.log('[Register] HTTP status:', status);
        console.log('[Register] Response message:', message);
        
        if (status === 409) {
          setError('Este email ya está registrado. ¿Necesitas iniciar sesión?');
        } else if (status === 400) {
          setError(message || 'Datos inválidos. Verifica los campos.');
        } else if (status === 500) {
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div>
              ...
              <input
                ...
                placeholder="Pizza Sur, El Buen Sabor, etc."
                disabled={isLoading}
              />
            </div>
